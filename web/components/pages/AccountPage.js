...
const AccountPage = class extends Component {
    ...
    onCancel = () => {
        // TODO popup modal asking why they want to cancel
    }

    render() {
        const { name } = this.state;
        const hasPaid = false; // Organisation has paid via chargebee
        const hasFreeTrial = hasPaid && true; // Organisation has paid via chargebee but is still within their free trial
        const hasFreeUse = true; // Organisation was created before payment options came in and therefore they have free usage (for now)
        return (
                ...
                <AccountProvider onSave={this.onSave} onRemove={this.onRemove}>
                    {({
                        ...
                        organisation
                            ...
                            <div>
                                {!hasPaid && (
                                    <div className="pricing" style={{ backgroundColor: 'white' }}>
                                ...
                                {hasPaid ? (hasFreeTrial ? (
                                    <div>
                                        <h2 className="text-center margin-bottom">Your organisation is currently trialling the Startup plan</h2>
                                        <p className="text-center">You have 29 days remaining. Click <a onClick={this.cancel}>here</a> if you'd like to cancel your trial</p>
                                    </div>
                                ) : (
                                    <div>
                                        <h2 className="text-center margin-bottom">Your organisation is on the Startup plan</h2>
                                        <p className="text-center">Click <a onClick={this.cancel}>here</a> if you'd like to cancel your plan</p>
                                    </div>
                                )) : null}
                            </div>
                        )
...
module.exports = (props) => (
    <WrappedAccountPage {...props} asyncScriptOnLoad={() => {
        Chargebee.init({
